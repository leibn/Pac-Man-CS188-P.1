    ...
    def getSuccessors(self, state: Any):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if hitsWall is False:
                checkd_bollean = list()
                # checkd_bollean is like the corner_state in places 1-4 - a boolean indicator,
                ...
                for i in range(len(self.corners)):
                    if ((nextx, nexty) == self.corners[i]) or state[i + 1]:
                        ...
                        checkd_bollean.append(True)
                    else:
                        # means not visited
                        checkd_bollean.append(False)
                # appending type:(successor((pacmanPosition,corner1status, corner2status, corner3status, corner4status))
                # ,action, stepCost)
                successors.append((((nextx, nexty), checkd_bollean[0], checkd_bollean[1], checkd_bollean[2],
                                    checkd_bollean[3]), action, 1))
        self._expanded += 1 # DO NOT CHANGE
        return successors
...
def recursive_path(not_visted_corners, currentPosition, min_path=sys.maxsize):
    ...
    Since the number of corners("standard" corner layout) is a finite number (and not large at all[4]), it is possible
        to solve an easier problem that will be with the Manhattan distance of the corners that have not yet been
        visited and without further restrictions on the possibility of reaching them. And since this is a:
            1.fully observable 2.single agent 3.deterministic 4.episodic 5.static 6.discrete and 7.well-known problem
        then there is no problem to look here for the best result without the use of special algorithms , and because of
        that, this is good heuristic for the real problem.
    And we will find the shortest way from Pacman's current location to all the corners we have not yet visited using
    a direct calculation like in the introduction to cs course.
    """
    ...
    for corner in not_visted_corners:
        ...
        if path < min_path: # otherwaise it will not update the min_path
            path = path + recursive_path([c for c in not_visted_corners if c != corner], corner, min_path)
            # souch that it will be the lower bound that can achieved from the state to a goal of the problem
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    if state[1:] == (True, True, True, True):  # The path of the remaining way is 0 because this is the goal state in this problem
        return 0
    not_visted_corners = list()
    for i in range(1, 5, 1):
        if not state[i]: # mean the boolean indicator is false
            not_visted_corners.append(corners[i - 1]) # corners are in 0-3 index and corner_state(inside State) are in 1-4 index's
        ...
        for firstFood in foodList:
            for secondFood in foodList:
                V12 = util.manhattanDistance(firstFood, secondFood)
                 # The huristic using the relaxed problam - the manhattan distance between two food's that f(n)=h(n)+g(n)<=f*(n)
                if V12 >= heuristic:
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState: pacman.GameState):
        ...
        # we already solve souch type of problem - using the uniform cost search we made in search.py
